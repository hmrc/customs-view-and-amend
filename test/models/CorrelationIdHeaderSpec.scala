...
class CorrelationIdHeaderSpec extends SpecBase {
  ...
  "CorrelationIdHeader" should {
    "create X-Correlation-ID header" in {
      ...
      CorrelationIdHeader.from("GB0000000123", Some(uuid.toString())) match {
        case ("X-Correlation-ID", v) =>
          assert(v.startsWith(Hash("GB0000000123").take(8)))
          assert(v.contains(uuid.toString.drop(8).take(10)))
          UUID.fromString(v)
      ...
      CorrelationIdHeader.from("GB0000000123", None) match {
        case ("X-Correlation-ID", v) =>
          assert(v.startsWith(Hash("GB0000000123").take(8)))
          UUID.fromString(v)
        case _                       => fail()
      ...
      CorrelationIdHeader.from("GB0000000123", Some("foo-bar-0123456789")) match {
        case ("X-Correlation-ID", v) =>
          assert(v.startsWith(Hash("GB0000000123").take(8)))
      ...
      CorrelationIdHeader.from("GB0000000123", Some("session-b3fa06aa-9375-4ad2-8ae1-2f09b06bed46")) match {
        case ("X-Correlation-ID", v) =>
          assert(v.startsWith(Hash("GB0000000123").take(8)))
